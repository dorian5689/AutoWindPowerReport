    ...
    def __init__(self, username=None, password=None, wfname=None, userid=None, wfname_id=None, start_run_time=None):
        ...
        self.message_dl = {
            ...
            "markdown": {
                "title": "OMS推送",
                "text":
                    F'第{self.wfname_id}个场站:{self.wfname}--已上报--电量--河南省安阳市--现场'
            }
        }
        self.message_cn = {
            "msgtype": "markdown",
            "markdown": {
                "title": "OMS推送",
                "text":
                    F'第:{self.wfname_id}个场站:{self.wfname}--已上报--储能--河南省安阳市--现场'
            }
        }
    ...
    def report_load_dl(self, table0, henan_oms_data):
        ...
        message_dl = {
            ...
            "markdown": {
                "title": "OMS推送",
                "text":
                    F'第{self.wfname_id}个场站:{self.wfname}--已上报--电量--河南省安阳市--现场<br>'
                    F'发电量:{fdl}<br>上网电量:{swdl}<br>弃电量:{qdl}<br>',
            }
        }
        # self.logger.warning(f'{message_dl}')
        # 测试电量专用
        # self.send_ding_dl_true_or_false(table0, message_dl=message_dl)
        # pass
        self.message_dl["markdown"] = message_dl["markdown"]
    ...
    def report_load_cn(self, table0, henan_oms_data):
        ...
        message_cn = {
            ...
            "markdown": {
                "title": "OMS推送",
                "text":
                    F'第{self.wfname_id}个场站:{self.wfname}--已上报--储能--河南省安阳市--现场<br>'
